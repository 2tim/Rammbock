...
class TestBinaryConversions(TestCase):
    ...
    def test_integer_to_bin(self):
        self.assertEqual(to_bin(0), '\x00')
        self.assertEqual(to_bin(5), '\x05')
        self.assertEqual(to_bin(255), '\xff')
        self.assertEqual(to_bin(256), '\x01\x00')
        self.assertEqual(to_bin(18446744073709551615), '\xff\xff\xff\xff\xff\xff\xff\xff')
    ...
    def test_binary_to_bin(self):
        self.assertEqual(to_bin('0b0'), '\x00')
        self.assertEqual(to_bin('0b1'), '\x01')
        self.assertEqual(to_bin('0b1111 1111'), '\xff')
        self.assertEqual(to_bin('0b1 0000 0000'), '\x01\x00')
        self.assertEqual(to_bin('0b01 0b01 0b01'), '\x15')
        self.assertEqual(to_bin('0b11' * 32), '\xff\xff\xff\xff\xff\xff\xff\xff')
        self.assertEqual(to_bin('0b11' * 1024), '\xff\xff\xff\xff\xff\xff\xff\xff' * 32)

    def test_hex_to_bin(self):
        self.assertEqual(to_bin('0x0'), '\x00')
        self.assertEqual(to_bin('0x00'), '\x00')
        self.assertEqual(to_bin('0x5'), '\x05')
        self.assertEqual(to_bin('0xff'), '\xff')
        self.assertEqual(to_bin('0x100'), '\x01\x00')
        self.assertEqual(to_bin('0x01 0x02 0x03'), '\x01\x02\x03')

    def test_integer_larger_than_8_bytes_works(self):
        self.assertEqual(to_bin('18446744073709551616'), '\x01\x00\x00\x00\x00\x00\x00\x00\x00')

    def test_hex_larger_than_8_bytes_works(self):
        self.assertEqual(to_bin('0xcafebabe f00dd00d deadbeef'), '\xca\xfe\xba\xbe\xf0\x0d\xd0\x0d\xde\xad\xbe\xef')

    def test_to_bin_of_length(self):
        self.assertEqual(to_bin_of_length(1, 0), '\x00')
        self.assertEqual(to_bin_of_length(2, 0), '\x00\x00')
        self.assertEqual(to_bin_of_length(3, 256), '\x00\x01\x00')
        self.assertRaises(AssertionError, to_bin_of_length, 1, 256)

    def test_to_hex(self):
        self.assertEqual(to_hex('\x00'), '00')
        self.assertEqual(to_hex('\x00\x00'), '0000')
        self.assertEqual(to_hex('\x00\xff\x00'), '00ff00')
        self.assertEqual(to_hex('\xca\xfe\xba\xbe\xf0\x0d\xd0\x0d\xde\xad\xbe\xef'), 'cafebabef00dd00ddeadbeef')

    def test_to_0xhex(self):
        self.assertEqual(to_0xhex('\x00'), '0x00')
        self.assertEqual(to_0xhex('\xca\xfe\xba\xbe\xf0\x0d\xd0\x0d\xde\xad\xbe\xef'), '0xcafebabef00dd00ddeadbeef')

    def test_to_0bbinary(self):
        self.assertEqual(to_binary_string_of_length(1, '\x00'), '0b0')
        self.assertEqual(to_binary_string_of_length(3, '\x00'), '0b000')
        self.assertEqual(to_binary_string_of_length(9, '\x01\xff'), '0b111111111')
        self.assertEqual(to_binary_string_of_length(12, '\x01\xff'), '0b000111111111')
        self.assertEqual(to_binary_string_of_length(68, '\x01\x00\x00\x00\x00\x00\x00\x00\x00'), '0b0001' + ('0000' * 16))
        self.assertEqual(to_binary_string_of_length(2048, '\xff\xff\xff\xff\xff\xff\xff\xff' * 32), '0b' + ('11' * 1024))
    ...
    def test_to_tbcd_binary(self):
        ...
        self.assertEqual(to_bin('0b0110001000010010000000100000000000000000000000000000000011110001'), to_tbcd_binary('262120000000001'))
